...
import re
#搜狗图片
img_url="https://x.xmoon.best/wp-content/uploads/2023/04/067__Chapter_3_066-1024x576"

'''  拓展：如何使用HTTPS请求
#HTTPS请求
import ssl
client = ssl.wrap_socket(socket.socket())   #ssl.wrap_socket 一个装饰器
client.connect(('x.xmoon.best',443))
'''

client = socket.socket()
# 创建连接              注意上面我们爬的是https协议的url，但是我们使用http也行的原因是自动进行了重定向
client.connect(("x.xmoon.best",80))     #连接服务器，ip地址的映射可以定位到它的服务器
